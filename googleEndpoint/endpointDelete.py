...
def getResourceID(deviceID):
    ...
    try:
        endpointDelete(results['mobiledevices'][0]['resourceId'])
    except:
        errorsValues.append([deviceID,"Fail(Windows/Mac)"])
        

# API to account wipe

def endpointDelete(rsrcID):
    #Building Required API Service
    service = build('admin', 'directory_v1', credentials=creds)

    # Call the Admin SDK Directory API
    try:
        results = service.mobiledevices().delete(customerId='my_customer',resourceId=rsrcID).execute()
